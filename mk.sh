...
export MAVEN_OPTS="-Xmx512m -XX:MaxPermSize=350m -Dhttps.protocols=TLSv1.2,TLSv1.3  -Djava.security.properties=../java.security -Djdk.tls.namedGroups=secp256r1,secp384r1 -Djdk.tls.disabledNameGroups='X25519,X448' -Djdk.httpcleint.HttpClient.log=all -Djdk.internal.httpclient.disableALPN=true -X logs" 
...
export _JAVA_OPTIONS='-Djava.io.tmpdir=/opt/mvnd-tmp -cp "/opt/java-libs/*.jar" -Djdk.lang.Process.launchMechanism=vfork -Djava.security.properties=../java.security'

 sudo bash /opt/maven-mvnd-1.0.2-linux-amd64/bin/mvnd.sh --debug clean package -Djava.io.tmpdir=/opt/mvnd-tmp   -Djdk.tls.client.cipherSuites='TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256' -Djavax.net.debug=ssl:handshake,verbose  $MAVEN_OPTS
